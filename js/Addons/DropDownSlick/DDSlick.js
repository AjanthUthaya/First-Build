// Start of teacher dropdown
(function(e) {
   e.fn.ddslickTeacher = function(l) {
      if (c[l]) {
...
// Start of vgs dropdown
(function(e) {
   e.fn.ddslickVGS = function(l) {
      if (c[l]) {
         return c[l].apply(this, Array.prototype.slice.call(arguments, 1))
      } else {
         if (typeof l === "object" || !l) {
...
// Start of multi-select dropdown
(function(e) {
   e.fn.ddslick_multi = function(l) {
      if (c[l]) {
         return c[l].apply(this, Array.prototype.slice.call(arguments, 1))
      } else {
         if (typeof l === "object" || !l) {
   ...
   c.init = function(l) {
      ...
      return this.each(function() {
         ...
         if (!q) {
            ...
            p.replaceWith(s);
            p = s;
            p.addClass("dd-container dd-multi").append(i).append(a);
            var n = p.find(".dd-select"),
               u = p.find(".dd-options");
            u.css({
               width: l.width
            ...
            e.each(l.data, function(v, w) {
               ...
               u.append('<li><a class="dd-option">' + (w.value ? ' <input class="dd-option-value" type="checkbox" style="display:none;" value="' + w.value + '" />' : "") + (w.imageSrc ? ' <img class="dd-option-image' + (l.imagePosition == "right" ? " dd-image-right" : "") + '" src="' + w.imageSrc + '" />' : "") + (w.text ? ' <label class="dd-option-text">' + '<span class="Filter-Checkbox-Box"></span>' + '<span>' + w.text + '</span>' + "</label>" : "") + (w.description ? ' <small class="dd-option-description dd-desc">' + w.description + "</small>" : "") + "</a></li>")
            });
            var t = {
               settings: l,
               original: m,
            ...
            p.find(".dd-option").on("click.ddslick", function(apple) {
               j(p, e(this).closest("li").index())

               // Check input after user clicks on li item
               GetInput = $(this).closest('li').find('.dd-option-value');
               GetCheckbox = $(this).closest('li').find('.Filter-Checkbox-Box');
               if (GetInput.prop('checked')) {
                  GetInput.prop('checked', false);
                  GetCheckbox.removeClass('fa fa-check').css('background', 'white');
               } else {
                  GetInput.prop('checked', true);
                  GetCheckbox.addClass('fa fa-check').css('background', '#14679e');
               }
