    ...
    t.config = {
      ...
      if (!e.config.touch || e.config.touch_tooltip) {
        ...
        if (!this.isTooltip(d)) {
          ...
          document.documentElement.clientWidth - _ - c < 0 ? (s.removeAttribute ? s.removeAttribute("left") : s.removeProperty("left"), s.right = document.documentElement.clientWidth - _ + 2 * (n.config.delta_x || 0) + "px") : 0 > _ ? s.left = o.x + Math.abs(n.config.delta_x || 0) + "px" : s.left = _ + "px",
